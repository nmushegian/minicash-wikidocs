...
- A `ment` is a list with 2 items: `[code, cash]`
	...
	- `cash` is a 7-byte value representing amount of cash held in this output.
-
...
- Mint tick special case
	- A mint tick is the tick that contains the miner reward. It has a special form, and puts all the special case logic in one place.
		...
		- A mint tick must have only 1 move.
		...
		- A mint tick must have only 1 ment.
			- The ment's `code` is a regular address, this is the miner's payout address.
			- The ment's `cash` must be equal to total fees in the tock, plus the subsidy. See [[supply]] for details about the distribution schedule.
		-